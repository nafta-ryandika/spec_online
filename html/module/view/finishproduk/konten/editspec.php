...
<div class="row">
    <div class="col-md-4">
        <div class="form-group">
            ...
            <input type="text" class="form-control" required="" id="SPEC_CODE" name="SPEC_CODE" value="<?php echo $SPEC_CODE; ?>" data-parsley-required>
        </div>                           
    </div>
    <div class="col-md-4">
        <div class="form-group">
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <select name="KODE_BUYER" id="KODE_BUYER" required="" class="form-control" data-parsley-required>
                ...
                <?php
                $result = $db1->prepare("select * from m_buyer where KODE_PERUSAHAAN = '$KODE_PERUSAHAAN'"); 
                $result->execute();
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) 
                {
                    ?>
                    <option value="<?php echo $row["KODE_BUYER"]; ?>"<?php if($KODE_BUYER == $row["KODE_BUYER"]) { echo "selected"; } ?>><?php echo $row["NAMA_BUYER"]; ?></option>
                    <?php
                }
                ?>
            </select>
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <select name="KODE_MERK" id="KODE_MERK" required="" class="form-control" data-parsley-required>
                ...
                <?php
                $result = $db1->prepare("select * from m_merk"); 
                $result->execute();
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) 
                {
                    ?>
...
<div class="row">
    <div class="col-md-4">
        <div class="form-group">
            ...
            <select name="KODE_PRODUK" id="KODE_PRODUK" required="" class="form-control" data-parsley-required>
                <option value="">Choose Product Name</option>
                <?php
                $result = $db1->prepare("select * from m_produk where KODE_PERUSAHAAN = '$KODE_PERUSAHAAN' AND STS_HAPUS = '0' order by NAMA_PRODUK asc"); 
                $result->execute();
                while ($row = $result->fetch(PDO::FETCH_ASSOC)) 
                {
                    ?>
                    <option value="<?php echo $row["KODE_PRODUK"]; ?>"<?php if($KODE_PRODUK == $row["KODE_PRODUK"]) { echo "selected"; } ?>><?php echo $row["NAMA_PRODUK"]; ?></option>
                    <?php
                }
                ?>
            </select>
    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <select name="KODE_BRAND" id="KODE_BRAND" required="" class="form-control" data-parsley-required>
